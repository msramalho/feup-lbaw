{{Log::info("serving flag_comment admin page")}}
        ...
        <div class="row">
            ...
            <div class="col-md-6 col-sm-12">
                <div class="float-right">
                    <a href="flagUsers" type="button" class="btn btn-primary btn-lg" >User Flags</a>   
                    <a href="flagPosts"type="button" class="btn btn-primary btn-lg">Post Flags</a>   
                    <a href="flagComments"type="button" class="btn btn-secondary btn-lg">Comment Flags</a>   
                </div>
            </div>
        ...
        <table class="table table-sm table-striped table-bordered text-center">
            ...
            <tbody id="tbody_cities">
				@foreach ($flags as $flag)
					...
					<tr data-id="{{$flag->flagger_id}},{{$flag->comment_id}}">
						<th scope="row">{{date ('d-m-Y', strtotime($flag->date))}}</th>
						<td><a href="#">{{$flag->comment_id}}</a></td>
						<td><a href="/user/{{$flag->user->username}}"  target="_blank">{{$flag->user->username}}</a></td>
                        <td>{{$flag->reason}}</td>
                        <td>
							<a class="m-2 ajax-link"
                            onclick="if(confirm('Archive?')){ archiveFlagComment('{{$flag->flagger_id}}','{{$flag->comment_id}}'); }" title="Archieve Flag"><i class="far fa-file-archive"></i></a>
						</td>
					</tr>
				@endforeach
            </tbody>
